...
export default function App() {
  ...
  const speak = useCallback((text: string) => {
    if (!soundEnabled || !('speechSynthesis' in window) || !language) return;
    ...
    if (voices.length > 0) {
        // Filter for voices that match the language (e.g. 'en', 'fr')
        const langPrefix = language === 'en' ? 'en' : language; 
        const availableVoices = voices.filter(v => v.lang.startsWith(langPrefix));
        
        if (availableVoices.length > 0) {
            // Sort to find the "best" voice
            availableVoices.sort((a, b) => {
                let scoreA = 0;
                let scoreB = 0;
                
                // Exact locale match
                if (a.lang === locale) scoreA += 5;
                if (b.lang === locale) scoreB += 5;
                
                // Quality keywords
                const qualityKeywords = ['Google', 'Premium', 'Enhanced', 'Natural'];
                if (qualityKeywords.some(k => a.name.includes(k))) scoreA += 3;
                if (qualityKeywords.some(k => b.name.includes(k))) scoreB += 3;

                // Specific preferences for English
                if (language === 'en') {
                    if (a.name.includes('Google US English')) scoreA += 10;
                    if (b.name.includes('Google US English')) scoreB += 10;
                    // Safari voices
                    if (a.name === 'Samantha') scoreA += 8;
                    if (b.name === 'Samantha') scoreB += 8;
                }
  ...
  const pickNewTarget = useCallback(() => {
    ...
    if (soundEnabled) {
        setTimeout(() => speak(`${ui.findQuestion} ${nameWithArticle} ?`), 500);
    }
  }, [gameState.targetId, soundEnabled, speak, actors, language, ui]);
  ...
  }, [language, pickNewTarget, gameState.mode, gameState.targetId, ui, actors]);
  ...
  const handleInteraction = useCallback((actor: ActorData) => {
    ...
    if (gameState.mode === GameMode.FIND && gameState.targetId) {
      if (actor.id === gameState.targetId) {
        ...
        speak(`${ui.bravo} ${ui.is} ${definiteName}. ${actor.sound}`);
        ...
        speak(`${ui.wrong} ${indefiniteName}. ${ui.weLookFor} ${targetName}.`);
        ...
        speak(`${definiteName} : ${actor.sound}`);
    }
  }, [gameState.mode, gameState.targetId, pickNewTarget, speak, discoveredIds, language, actors, ui]);
  ...
  const updatePosition = useCallback(() => {
    if (!language) return; // Don't move if menu is open
    ...
    // Use ZQSD for French (AZERTY), WASD for English/Polish (QWERTY)
    ...
    // Support standard WASD (Right=D) and requested "WASH" (Right=H) for non-French
  ...
  }, [language, nearbyActorId, handleInteraction, actors]);
  ...
  if (!language) {
      return <LanguageMenu onSelect={setLanguage} />;
  }
        ...
        <div className="bg-white/60 backdrop-blur-md border-2 border-white/50 pl-6 pr-2 py-2 rounded-full shadow-lg flex items-center gap-4 pointer-events-auto transition-all hover:scale-105 max-w-[90vw] md:ml-auto">
            <div className="flex flex-col">
                <div className={`text-lg md:text-xl font-bold leading-none ${
                    gameState.messageColor === 'success' ? 'text-green-600' :
            ...
            <button 
                onClick={toggleMode}
                className={`ml-4 px-4 py-2 rounded-full text-sm font-bold shadow-sm transition-colors ${
                    gameState.mode === GameMode.FIND ? 'bg-sky-blue text-white' : 'bg-sage-green text-white'
                }`}
            >
